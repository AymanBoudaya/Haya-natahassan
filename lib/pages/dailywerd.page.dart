...
import '../data/dailywerd_data.dart'; // Ensure this matches your data source
import '../widgets/customappbar.page.dart';
import '../widgets/theme.provider.dart'; // For SystemChrome

class DailyWerdPage extends StatefulWidget {
  const DailyWerdPage({super.key});

  @override
  State<DailyWerdPage> createState() => _DailyWerdPageState();
}

class _DailyWerdPageState extends State<DailyWerdPage> {
  ...
  Future<void> _loadTapCounts() async {
    ...
    final savedCounts = prefs.getStringList('werdtapCounts') ?? [];
    setState(() {
  ...
  Future<void> _saveTapCounts() async {
    final prefs = await SharedPreferences.getInstance();
    final savedCounts = _tapCounts.entries.map((e) => e.value.toString()).toList();
    await prefs.setStringList('werdtapCounts', savedCounts);
  }

  Future<void> _resetTapCounts() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove('werdtapCounts');
    setState(() {
      _tapCounts.clear();
  ...
  double get _completionPercentage {
    final totalCards = dailywerds.length;
    final completedCards = _tapCounts.entries.where((entry) {
      final index = entry.key;
      final tapCount = entry.value;
      final targetTaps = dailywerds[index]["targetTaps"] as int;
  ...
  bool get _allCardsCompleted {
    return dailywerds.length > 0 && dailywerds.length == _tapCounts.entries.where((entry) {
      final index = entry.key;
      final tapCount = entry.value;
      final targetTaps = dailywerds[index]["targetTaps"] as int;
      return tapCount >= targetTaps;
  ...
  void _handleTap() {
    setState(() {
      final targetTaps = dailywerds[_currentIndex]["targetTaps"] as int;
  ...
  Widget build(BuildContext context) {
    ...
    final fontSize = themeProvider.themeData.textTheme.bodyLarge?.fontSize ?? 24;
            ...
            child: Stack(
              children: [
                CarouselSlider(
                  ...
                  items: dailywerds.asMap().entries.map((entry) {
                    int index = entry.key;
                    Map<String, dynamic> dailyWerdItem = entry.value;
                    int tapCount = _tapCounts[index] ?? 0;
                    int targetTaps = dailyWerdItem["targetTaps"] as int;
                                    ...
                                    children: [
                                      Text(
                                        ...
                                        style: TextStyle(
                                          ...
                                          color: isCompleted
                                              ? Colors.grey[600]
                                              : textColor, // Use the theme's text color
                                        ),
                                      ),
                                      Container(
                                        padding: EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                                        decoration: BoxDecoration(
                                          color: tapCount >= targetTaps ? Colors.green : Colors.red,
                  ...
                  child: Padding(
                    ...
                    child: InkWell(
                      onTap: (_tapCounts[_currentIndex] ?? 0) >=
                          (dailywerds[_currentIndex]["targetTaps"] as int)
                          ? null
                          : _handleTap,
